...
def exactly?(array, num, &block)
    (array.count { |ele| block.call(ele) }) == num
end
...
def xnor_select(array, prc1, prc2)
    array.select {|ele| selected << ele if (prc1.call(ele) && prc2.call(ele)) || (!prc1.call(ele) && !prc2.call(ele))}
end
...
def multi_map(array, optNum = 1, &block)
    mapped = []   
    array.each do |ele| 
        optNum.times { ele = block.call(ele) } 
        mapped << ele
    end
    mapped
end
...
def selected_map!(array, prc1, prc2)
    array.each.with_index do |ele, idx|
        array[idx] = prc2.call(ele) if prc1.call(ele)
    end
    nil
end
...
def proc_suffix(sentence, hash)
    ...
    words.each do |word|
        new_word = word
        hash.each do |proc, value|
            new_word += value if proc.call(word)
        end
    new_words << new_word
    end
    new_words.join(" ")
...
def proctition_platinum(array, *procs)
    partition = {}
    procs.each_index { |i| partition[i+1] = [] }
    array.each do |ele|
