...
class Array
  ...
  def bubble_sort(&prc)
    copyself = self.dup
    copyself.bubble_sort!(&prc)
    
  end
end
#p [1, 3, 5, 2].bubble_sort! { |num1, num2| num1 <=> num2 }
#p [1, 3, 5, 2].bubble_sort { |num1, num2| num1 <=> num2 }
...
def substrings(string)
  ...
  (0..string.length-1).each do |idx1|
    subs << string[idx1]
    (0..string.length-1).each do |idx2|
      subs << string[idx1..idx2] if idx1<idx2 
    end
  end
  subs#.uniq
end
...
class Array
  ...
  def my_inject (accumulator = self.first, &blk)
    self.each do |item|
      accumulator = blk.call(accumulator,item)
    end
    accumulator
  end

end
#p [1, 2, 3].my_map { |ele| ele*10 }
#p [1, 2, 3].my_select { |ele| ele ==2 }
#p [1, 2, 3, 5].my_inject { |sum, ele| sum * ele }
#p [1, 2, 3, 5].my_inject { |sum, ele| ele + sum } Wrong output, couldn't solve.
